...
def validate_sources():
    for base in SOURCES.values():
        for size in SIZES.values():
            ...
            if not p.is_file() or p.stat().st_size==0: raise RuntimeError(f"Missing approved source asset: {p}")
            data=p.read_bytes()
...
def replace_image_refs(html):
    for name,base in SOURCES.items():
        for variant,size in SIZES.items():
            html=re.sub(rf'assets/images/{re.escape(name)}-{variant}\.webp(?:\?v=[^\s\"\']+)?',f'assets/optimized/{base}-{size}.webp?v={ASSET_VERSION}',html)
...
def clean_preloads(html):
    ...
    pre='<link rel="preload" as="image" href="assets/optimized/warehouse-hero-1600.webp?v=20260915" fetchpriority="high" media="(min-width:851px)"><link rel="preload" as="image" href="assets/optimized/warehouse-hero-640.webp?v=20260915" fetchpriority="high" media="(max-width:850px)">'
