...
def optimize_performance_markup(html: str) -> str:
    # Keep srcset width descriptors truthful to the generated binary dimensions.
    ...
    # The source is the same image as src; removing onerror avoids a pointless handler.
    ...
    html = re.sub(r'(assets/images/(?:food-products|plastic-products|sweets-snacks)-desktop\\.webp" alt=)([^>]*?)', r'\\1\\2', html)
    # Accurate intrinsic dimensions reserve the correct aspect ratio and reduce layout uncertainty.
    ...
    # Preload the LCP hero image using the same breakpoint strategy as the CSS background.
    ...
    # Use the smaller portrait asset on phones so the hero does not download a desktop image.
    desktop_bg = "background:linear-gradient(90deg,rgba(4,20,42,.88),rgba(7,26,54,.6) 48%,rgba(7,26,54,.18)),url('assets/images/warehouse-desktop.webp') center/cover"
    ...
    if desktop_bg in html and mobile_bg not in html:
...
def rewrite_index() -> None:
    ...
    if "assets/images/warehouse-desktop.webp" not in html or "assets/images/warehouse-mobile.webp" not in html:
        raise RuntimeError("Warehouse responsive assets are not referenced")
    ...
    for required in ('food-products-mobile.webp 750w', 'food-products-tablet.webp 1024w', 'food-products-desktop.webp 1920w', 'warehouse-mobile.webp'):
        if required not in html:
            raise RuntimeError(f"Performance optimization missing: {required}")
    INDEX.write_text(html, encoding="utf-8")
    print("Updated index.html: local responsive WebP, managed SEO, accurate srcset widths, and LCP hero loading")
